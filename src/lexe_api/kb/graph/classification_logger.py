...
class ClassificationLog:
    ...
    llm_candidate_set: list[str] | None = None
    llm_output_label: str | None = None
    llm_output_confidence: float | None = None
    llm_accepted: bool | None = None
    llm_agreement: bool | None = None  # True se i due LLM concordano
    llm_judge_called: bool | None = None
...
class ClassificationLogger:
    ...
    def log(self, classification_log: ClassificationLog) -> None:
        ...
        if self._file is None:
            raise RuntimeError("Logger not opened. Use 'with ClassificationLogger(...) as logger:'")
...
class ClassificationStats:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "latency_l1_p50": float(np.percentile(self.latencies_l1, 50))
            if self.latencies_l1
            else 0,
            "latency_l1_p95": float(np.percentile(self.latencies_l1, 95))
            if self.latencies_l1
            else 0,
            "latency_llm_p50": float(np.percentile(self.latencies_llm, 50))
            if self.latencies_llm
            else 0,
            "latency_llm_p95": float(np.percentile(self.latencies_llm, 95))
            if self.latencies_llm
            else 0,
        }
