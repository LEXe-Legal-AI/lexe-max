...
MODELS = {
    "free": "tngtech/deepseek-r1t2-chimera:free",         # Free - R1T2 hybrid 671B
    "chimera": "tngtech/deepseek-r1t2-chimera",           # Paid - R1T2 hybrid 671B ($0.3/$1.2 per M)
    "lite": "google/gemini-2.5-flash-lite",               # Economico - Gemini 2.5 Flash Lite
    "medium": "google/gemini-2.5-flash-lite",             # Economico - Gemini 2.5 Flash Lite
    "good": "mistralai/mistral-large-2512",               # Mistral Large 2512
}
...
def extract_articles(text: str, matches: list[ArticleMatch]) -> list[ExtractedArticle]:
    ...
    for i, h in enumerate(unique_headers):
        ...
        rubrica = None
        testo_start = 1  # Default: testo inizia dalla seconda riga

        # Cerca potenziale rubrica nelle prime righe
        for j, line in enumerate(lines[1:], 1):
            line_stripped = line.strip()
            if not line_stripped or len(line_stripped) <= 5:
                continue

            # Rubrica: breve (< 80 chars), capitalizzata, senza pattern di testo
            is_short = len(line_stripped) < 80
            is_capitalized = line_stripped[0].isupper() or line_stripped.startswith('(')
            # Pattern che indicano testo, non rubrica
            text_patterns = [
                line_stripped[0].islower(),  # inizia minuscola
                ' è ' in line_stripped,      # verbo essere
                ' sono ' in line_stripped,   # verbo essere plurale
                ' si ' in line_stripped,     # pronome riflessivo
                line_stripped.endswith('.'), # frase completa
                ',' in line_stripped and len(line_stripped) > 60,  # frase con virgole
            ]
            looks_like_text = any(text_patterns)

            if is_short and is_capitalized and not looks_like_text:
                rubrica = line_stripped.rstrip('.')
                testo_start = j + 1
            break

        # Estrai testo
        testo = '\n'.join(lines[testo_start:]).strip()

        # Fix: se testo è vuoto ma rubrica esiste, la "rubrica" è il testo
        if not testo and rubrica:
            testo = rubrica
            rubrica = None
...
def convert_pdf(pdf_path: str, use_ocr: bool = False) -> str:
    ...
    if use_ocr:
        print(f"    OCR: enabled (GPU accelerated)")
...
def run_pipeline(
    ...
    stats = {"timings": {}, "counts": {}, "options": {"ocr": use_ocr}}

    # 0. Convert PDF
    print(f"\n[0] Converting PDF{'  (OCR enabled)' if use_ocr else ''}...")
    start = time.time()
    text = convert_pdf(pdf_path, use_ocr=use_ocr)
...
def main():
    ...
    parser.add_argument("--ocr", action="store_true", help="Enable OCR (GPU accelerated)")
