#!/usr/bin/env python3
"""
Cross-validate normativa articles via LEXE Tools API.

Usa l'API lexe-tools-it deployata su staging invece di chiamare
Normattiva direttamente. Il server Hetzner fa da proxy.

Usage:
    cd lexe-max

    # Via staging API (default)
    uv run python scripts/cross_validate_via_api.py --codice CC --limit 100

    # Via localhost (se lexe-tools running localmente)
    uv run python scripts/cross_validate_via_api.py --codice CC --limit 10 --api-url http://localhost:8021
"""

import asyncio
import argparse
import hashlib
import random
import re
from datetime import datetime

import asyncpg
import httpx
import structlog

logger = structlog.get_logger()

# Database connection
DB_URL = "postgresql://lexe_kb:lexe_kb_dev_password@localhost:5434/lexe_kb"

# Default API URL (staging on Hetzner)
DEFAULT_API_URL = "https://api.lexe.pro"  # or http://localhost:8021 for local

# Mapping codice -> act_type per API
CODICE_TO_ACT_TYPE = {
    "CC": "codice civile",
    "CP": "codice penale",
    "CPC": "codice procedura civile",
    "CPP": "codice procedura penale",
    "CDS": "codice della strada",
    "COST": "costituzione",
}


def normalize_text(text: str) -> str:
    """Normalizza testo per confronto."""
    text = text.lower()
    text = re.sub(r'\s+', ' ', text)
    text = text.replace('"', '"').replace('"', '"').replace("'", "'").replace("'", "'")
    text = text.replace('–', '-').replace('—', '-')
    text = text.strip()
    return text


def compute_hash(text: str) -> str:
    """SHA256 del testo normalizzato."""
    return hashlib.sha256(normalize_text(text).encode('utf-8')).hexdigest()


async def fetch_from_lexe_api(
    client: httpx.AsyncClient,
    api_url: str,
    codice: str,
    articolo: str
) -> tuple[str | None, str | None, bool | None]:
    """
    Fetch articolo via LEXE Tools API.

    Returns:
        Tuple di (testo, urn, is_vigente) o (None, None, None) se non trovato
    """
    act_type = CODICE_TO_ACT_TYPE.get(codice)
    if not act_type:
        logger.warning("Codice not supported", codice=codice)
        return None, None, None

    # Call lexe-tools-it API
    endpoint = f"{api_url}/api/v1/tools/normattiva/search"

    payload = {
        "act_type": act_type,
        "article": articolo,
    }

    try:
        response = await client.post(
            endpoint,
            json=payload,
            timeout=30.0
        )

        if response.status_code == 404:
            return None, None, None

        if response.status_code != 200:
            logger.warning("API error", status=response.status_code, body=response.text[:200])
            return None, None, None

        data = response.json()

        if not data.get("success"):
            return None, None, None

        text = data.get("text", "")
        urn = data.get("urn", "")
        is_vigente = data.get("vigente", True)

        return text, urn, is_vigente

    except httpx.TimeoutException:
        logger.warning("API timeout", endpoint=endpoint)
        return None, None, None
    except Exception as e:
        logger.error("API error", error=str(e))
        return None, None, None


async def validate_article(
    conn: asyncpg.Connection,
    client: httpx.AsyncClient,
    api_url: str,
    row: dict
) -> dict:
    """Valida singolo articolo contro fonte canonica via API."""
    codice = row['codice']
    articolo = row['articolo']
    db_text = row['testo']

    result = {
        'id': row['id'],
        'codice': codice,
        'articolo': articolo,
        'status': 'pending',
        'canonical_url': None,
        'canonical_hash': None,
        'hash_match': False,
        'diff_type': None,
        'is_vigente': None,
    }

    # Fetch from API
    canonical_text, canonical_urn, is_vigente = await fetch_from_lexe_api(
        client, api_url, codice, articolo
    )

    result['is_vigente'] = is_vigente

    if canonical_text is None:
        result['status'] = 'review_needed'
        result['diff_type'] = 'not_found_in_canonical'
        return result

    if canonical_urn:
        result['canonical_url'] = f"https://www.normattiva.it/uri-res/N2Ls?{canonical_urn}"

    # Compare hashes
    db_hash = compute_hash(db_text)
    canonical_hash = compute_hash(canonical_text)

    result['canonical_hash'] = canonical_hash
    result['hash_match'] = (db_hash == canonical_hash)

    if result['hash_match']:
        result['status'] = 'verified'
        result['diff_type'] = 'exact'
    else:
        # Jaccard similarity for diff classification
        db_words = set(normalize_text(db_text).split())
        can_words = set(normalize_text(canonical_text).split())

        intersection = len(db_words & can_words)
        union = len(db_words | can_words)
        similarity = intersection / union if union > 0 else 0

        if similarity > 0.95:
            result['status'] = 'verified'
            result['diff_type'] = 'format_diff'
        elif similarity > 0.80:
            result['status'] = 'format_diff'
            result['diff_type'] = 'minor'
        else:
            result['status'] = 'content_diff'
            result['diff_type'] = 'substantive'

    return result


async def update_validation_status(conn: asyncpg.Connection, result: dict) -> None:
    """Aggiorna stato validazione nel database con data validation."""
    diff_summary = f"hash_match={result['hash_match']}, diff_type={result['diff_type']}, vigente={result['is_vigente']}"

    await conn.execute("""
        UPDATE kb.normativa SET
            validation_status = $1,
            validated_at = NOW(),
            canonical_url = $2,
            canonical_hash = $3,
            validation_diff = $4,
            canonical_source = 'normattiva',
            canonical_retrieved_at = NOW()
        WHERE id = $5
    """, result['status'], result['canonical_url'], result['canonical_hash'],
         diff_summary, result['id'])


async def main():
    parser = argparse.ArgumentParser(description="Cross-validate via LEXE Tools API")
    parser.add_argument("--codice", default="CC", help="Codice to validate")
    parser.add_argument("--limit", type=int, default=0, help="Max articles (0=all)")
    parser.add_argument("--sample", type=float, default=0, help="Random sample rate (0-1)")
    parser.add_argument("--api-url", default=DEFAULT_API_URL, help="LEXE Tools API URL")
    parser.add_argument("--rps", type=float, default=2.0, help="Requests per second")
    parser.add_argument("--dry-run", action="store_true", help="Don't update database")
    args = parser.parse_args()

    print(f"\n{'='*60}")
    print(f"  CROSS-VALIDATION VIA LEXE TOOLS API")
    print(f"  {datetime.now().isoformat()}")
    print(f"  Codice: {args.codice}")
    print(f"  API: {args.api_url}")
    print(f"  Rate: {args.rps} req/sec")
    print(f"{'='*60}\n")

    # Test API connectivity
    async with httpx.AsyncClient() as test_client:
        try:
            health = await test_client.get(f"{args.api_url}/health/live", timeout=10.0)
            print(f"API Health: {health.status_code}")
        except Exception as e:
            print(f"WARNING: API not reachable: {e}")
            print("Continuing anyway...")

    # Connect to DB
    conn = await asyncpg.connect(DB_URL)
    print("Connected to database")

    # Get articles to validate
    rows = await conn.fetch("""
        SELECT id, codice, articolo, testo, validation_status
        FROM kb.normativa
        WHERE codice = $1
        ORDER BY articolo
    """, args.codice)
    print(f"Found {len(rows)} articles for {args.codice}")

    # Apply sampling
    if args.sample > 0:
        n_sample = max(1, int(len(rows) * args.sample))
        rows = random.sample(rows, n_sample)
        print(f"Sampling {n_sample} articles ({args.sample*100:.1f}%)")
    elif args.limit > 0:
        rows = rows[:args.limit]
        print(f"Limited to {len(rows)} articles")

    # Stats
    stats = {
        'total': len(rows),
        'verified': 0,
        'format_diff': 0,
        'content_diff': 0,
        'review_needed': 0,
        'errors': 0,
    }

    start_time = datetime.now()
    delay = 1.0 / args.rps

    async with httpx.AsyncClient() as client:
        for i, row in enumerate(rows, 1):
            try:
                result = await validate_article(conn, client, args.api_url, dict(row))

                stats[result['status']] = stats.get(result['status'], 0) + 1

                if not args.dry_run:
                    await update_validation_status(conn, result)

                # Progress
                elapsed = (datetime.now() - start_time).total_seconds()
                rate = i / elapsed if elapsed > 0 else 0
                eta = (len(rows) - i) / rate if rate > 0 else 0

                icon = "✓" if result['status'] == 'verified' else "~" if 'diff' in result['status'] else "!"
                print(f"  [{i}/{len(rows)}] {rate:.2f}/sec | ETA: {eta:.0f}s | {icon} Art. {row['articolo']}: {result['status']}")

                await asyncio.sleep(delay)

            except Exception as e:
                stats['errors'] += 1
                logger.error("Validation error", articolo=row['articolo'], error=str(e))

    await conn.close()

    # Final stats
    elapsed = (datetime.now() - start_time).total_seconds()

    print(f"\n{'='*60}")
    print(f"  RESULTS")
    print(f"{'='*60}")
    print(f"  Total validated: {stats['total']}")
    print(f"  Verified (exact): {stats['verified']}")
    print(f"  Format diff: {stats.get('format_diff', 0)}")
    print(f"  Content diff: {stats.get('content_diff', 0)}")
    print(f"  Review needed: {stats.get('review_needed', 0)}")
    print(f"  Errors: {stats['errors']}")
    print(f"  Time: {elapsed:.1f} seconds")
    print(f"  Dry run: {args.dry_run}")
    print(f"{'='*60}\n")


if __name__ == "__main__":
    asyncio.run(main())
